    ...
    public CalendarBox[][] fillInCalendar(int startDay, int numberOfDays, UIController controller) throws IOException {
        ...
        for (int row = 0; row < calendar.length ; row++) {
            for (int col = 0; col < calendar[row].length; col++) {
                ...
                CalendarBox box = new CalendarBox(dayInMonth,row,true,null);
...
//                }
    ...
    public  CalendarBoxRedone[][] CalendarNEWLoad(int startDay, int numberOfDays, JSON json)throws IOException{

        //Intitialize a general calendar array size
        CalendarBoxRedone[][] calendar = new CalendarBoxRedone[5][7];
        ...
        for (int row = 0; row < calendar.length ; row++) {
            for (int col = 0; col < calendar[row].length; col++) {
                ...
                CalendarBoxRedone box = new CalendarBoxRedone(dayInMonth,row,true,null);
                calendar[row][col] = box;
                dayInMonth++;
